...
public class CommentDao extends AbstractMFlixDao {
	...
	public boolean updateComment(String commentId, String text, String email) {
		...
		// updating an
		// user own comments
		Bson commentM = new Document("_id", new ObjectId(commentId)).append("email", email);
		Comment comment = commentCollection.find(commentM).first();
		if (comment == null)
			return false;
		else {
			try {
				commentCollection.updateOne(commentM, combine(set("text", text), set("date", new Date())));
			} catch (MongoException e) {
